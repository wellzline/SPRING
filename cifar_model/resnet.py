...
#         else:
#             raise ValueError('Unsupported model name for ImageNet.')
...
# activation_fn = nn.Softplus(beta=5) 
...
class BasicBlock(nn.Module):
    ...
    # def forward(self, x):
    #     out = activation_fn(self.bn1(self.conv1(x)))
    #     out = self.bn2(self.conv2(out))
    #     out += self.shortcut(x)
...
class Bottleneck(nn.Module):
    ...
    # def forward(self, x):
    #     out = activation_fn(self.bn1(self.conv1(x)))
    #     out = activation_fn(self.bn2(self.conv2(out)))
    #     out = self.bn3(self.conv3(out))
    #     out += self.shortcut(x)
    #     out = activation_fn(out)
    #     return out
